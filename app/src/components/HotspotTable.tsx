...
const DatePicker = DatePickerPkg.default || DatePickerPkg;
const DateObject = DatePickerPkg.DateObject || (DatePickerPkg.default && DatePickerPkg.default.DateObject);
...
import type { AccumulatedData } from "../core/model/hotspot";
...
export default function HotspotTable() {
  ...
  useEffect(() => {
    const getDataHotspot = async () => {
      try {
        ...
        if (latestDate) {
          try {
            const dateObj = new DateObject(latestDate);
            setDates([dateObj]);
          } catch {
            setDates([]);
          }
  ...
  const exportData = () => {
    ...
    if (dates.length === 0) {
      ...
      dateStr = dates[0] instanceof DateObject ? dates[0].format("YYYY-MM-DD") : new Date().toISOString().split("T")[0];
    } else {
      const sortedDates = [...dates].sort((a, b) => {
        const dateA = a instanceof DateObject ? new Date(a.format("YYYY-MM-DD")).getTime() : 0;
        const dateB = b instanceof DateObject ? new Date(b.format("YYYY-MM-DD")).getTime() : 0;
        return dateA - dateB;
      });
      const firstDate = sortedDates[0] instanceof DateObject ? sortedDates[0].format("YYYY-MM-DD") : new Date().toISOString().split("T")[0];
      const lastDate = sortedDates[sortedDates.length - 1] instanceof DateObject
        ? sortedDates[sortedDates.length - 1].format("YYYY-MM-DD")
        : new Date().toISOString().split("T")[0];
      dateStr = `${firstDate}_to_${lastDate}`;
    }
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                type="date"
                value={dates.length > 0 && dates[0] instanceof DateObject ? dates[0].format("YYYY-MM-DD") : ""}
                onChange={(e) => {
                  if (e.target.value) {
                    try {
                      const dateObj = new DateObject(e.target.value);
                      setDates([dateObj]);
                      setCurrentPage(1);
                    } catch {
                      setDates([]);
