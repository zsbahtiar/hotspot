...
import type { QueryData } from "../../core/model/query";
import type { Time } from "../../core/model/time";
    ...
    return (
      <div className="relative" ref={dropdownRefs[type]}>
        ...
        >
          ...
          {loading[type] ? (
            <RefreshCw width="16" height="16" className="text-green-600" style={{animation: 'spin 1s linear infinite'}} />
          ) : (
            showDropdown ? <ChevronUp className="text-gray-500" /> : <ChevronDown className="text-gray-500" />
          )}
        </div>
        {showDropdown && (
          <ul className="absolute z-20 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto custom-scrollbar">
            ...
            {loading[type] ? (
              <li className="p-2 text-center text-gray-500 flex items-center justify-center">
                <RefreshCw width="16" height="16" className="mr-2" style={{animation: 'spin 1s linear infinite'}} />{" "}
                Memuat data...
              </li>
            ) : dataList.length > 0 ? (
